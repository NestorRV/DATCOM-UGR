...
MiBoxPlot_IQR_Univariate_Outliers = function (datos, indice.de.columna, coef = 1.5){
  ...
  nombre.de.columna = colnames(datos)
  
  ggboxplot = ggplot(data = datos, 
                     aes(x=factor(""), 
                     y=datos[,indice.de.columna]) , 
                     environment = environment()) + 
    xlab(nombre.de.columna[indice.de.columna]) + ylab("") +
...
# en la función ggplot que tiene un bug de dise�o ya que dentro del parámetro aes
